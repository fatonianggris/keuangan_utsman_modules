...
class IncomeModel extends CI_Model {
    ...
    public function get_income_du_by_id($id = '') {
        $this->db2->select("p.*, 
                                ...
                                DATE_FORMAT(p.waktu_transaksi, '%d/%m/%Y') AS tgl_transaksi,
                                DATE_FORMAT(p.tanggal_invoice, '%d/%m/%Y') AS tgl_invoice,
                                MONTH(p.tanggal_invoice) AS bulan_invoice,
                                TIME(p.tanggal_invoice) AS waktu_invoice
                         ");
