...
class Income extends MX_Controller {
    ...
    public function list_income_du() {
        ...
        $data['income'] = $this->IncomeModel->get_all_income_du();
    ...
    public function list_income_dpb() {
        $data['nav_in'] = 'menu-item-here';
        $data['income'] = $this->IncomeModel->get_all_income_dpb();
            ...
            if (password_verify(($data['pass_verification']), $check_pass[0]->password)) {
                ...
                if ($this->googleCaptachStore($recaptchaResponse, $userIp) == 1) {
                    ...
                    if (isset($_FILES['file_tagihan_dpb']['name']) && in_array($_FILES['file_tagihan_dpb']['type'], $file_mimes)) {
                        ...
                        if ($import_data == true) {
                            $this->session->set_flashdata('flash_message', warn_msg("Peringatan!, File " . $_FILES['file_tagihan_dpb']['name'] . " Telah terbaca, Silahkan <b>PENGECEKAN & PERSETUJUAN</b> untuk mengimpor seluruh data file tersebut. Jika terjadi ketidaksamaan dengan Data Asli, dimohon untuk <b>UPLOAD ULANG</b>. Terima Kasih"));
            ...
            if (password_verify(($data['pass_verification']), $check_pass[0]->password)) {
                ...
                if ($this->googleCaptachStore($recaptchaResponse, $userIp) == 1) {
                    ...
                    if (isset($_FILES['file_tagihan_du']['name']) && in_array($_FILES['file_tagihan_du']['type'], $file_mimes)) {
                        ...
                        if ($import_data == true) {
                            $this->session->set_flashdata('flash_message', warn_msg("Peringatan!, File " . $_FILES['file_tagihan_du']['name'] . " Telah terbaca, Silahkan <b>PENGECEKAN & PERSETUJUAN</b> untuk mengimpor seluruh data file tersebut. Jika terjadi ketidaksamaan dengan Data Asli, dimohon untuk <b>UPLOAD ULANG</b>. Terima Kasih"));
