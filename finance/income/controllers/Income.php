...
class Income extends MX_Controller
{
    ...
    public function post_income_dpb()
    {
        ...
        if ($invoice == true) {
            ...
            if ($data['nomor_invoice'] == null || $data['nomor_pembayaran'] == null || $data['nomor_invoice'] == "" || $data['nomor_pembayaran'] == "") {
                ...
                $data['password']  = password_hash(paramEncrypt($data['nis']), PASSWORD_DEFAULT, ['cost' => 12]);
    ...
    public function update_income_du_transition($id = '')
    {
        ...
        if ($check_student) {
            ...
            if ($check_student_name_and_number) {
                ...
                $data['password'] = password_hash(paramEncrypt(trim($new_nis)), PASSWORD_DEFAULT, ['cost' => 12]);
            }
        } else {
            $data['password'] = password_hash(paramEncrypt($new_nis), PASSWORD_DEFAULT, ['cost' => 12]);
        }
    ...
    public function update_income_dpb_transition($id = '')
    {
        ...
        if ($check_student) {
            ...
            if ($check_student_name_and_number) {
                ...
                $data['password'] = password_hash(paramEncrypt(trim($data['nomor_bayar'])), PASSWORD_DEFAULT, ['cost' => 12]);
            }
        } else {
            $data['password'] = password_hash(paramEncrypt($data['nomor_bayar']), PASSWORD_DEFAULT, ['cost' => 12]);
        }
                    ...
                    if (isset($_FILES['file_tagihan_dpb']['name']) && in_array($_FILES['file_tagihan_dpb']['type'], $file_mimes)) {
                        ...
                        for ($z = 0; $z < count($sheetData[0]); $z++) {
                            if (strpos($sheetData[0][$z], 'rincian') !== false) {
                                $index_header_rincian[] = ['index_kolom' => $z, 'nama_kolom' => implode(' ', explode('_', explode('_', $sheetData[0][$z], 2)[1]))];
                            }
                        }
                        ...
                        for ($z = 0; $z < count($sheetData[0]); $z++) {
                            if (strpos($sheetData[0][$z], 'info') !== false) {
                                $index_header_info[] = ['index_kolom' => $z, 'nama_kolom' => implode(' ', explode('_', explode('_', $sheetData[0][$z], 2)[1]))];
                            }
                        }
                        ...
                        for ($i = 1; $i < count($sheetData); $i++) {
                            ...
                            if ($student) {
                                ...
                                if ($check_student_name_and_number) {
                                    ...
                                    $password = password_hash(paramEncrypt(trim($sheetData[$i]['1'])), PASSWORD_DEFAULT, ['cost' => 12]);
                                }
    ...
    public function add_ajax_grade($id_grad_lvl = '', $id_grad = '')
    {
        ...
        $query = $this->db2->get_where('tingkat', ['level_tingkat' => $id_grad_lvl]);
    ...
    public function list_filter($list, $args = [], $operator = 'AND')
    {
        if (! is_array($list)) {
            return [];
        }
